...
# 设置驱动路径
os.environ["CLASSPATH"] = "D:/wangq/Documents/gbasedbtjdbc_3.6.3_3X2_1_377ee9.jar"
...
def quick_gbase8s_test():
    ...
    # 创建连接URL
    url = (
        f"jdbcgbase8s://{config['host']}:{config['port']}/{config['database']}:GBASEDBTSERVER={config['server']}"
    ...
    with engine.connect() as conn:
        # 测试基本查询
        result = conn.execute(text("select dbinfo('version_gbase','full') from dual"))
