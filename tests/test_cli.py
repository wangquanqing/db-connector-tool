...
from db_connector_tool.cli import DBConnectorCLI, create_argument_parser, main
...
class TestDBConnectorCLI(unittest.TestCase):
    ...
    def test_execute_query(self):
        ...
        with mock.patch('db_connector_tool.cli.DatabaseManager') as mock_db_manager:
            with mock.patch('db_connector_tool.cli.DBConnectorCLI._display_results'):
                ...
                mock_instance.execute_query.assert_called_once_with("test_conn", "SELECT * FROM users")
    ...
    def test_execute_file(self):
        ...
        with mock.patch('os.path.exists', return_value=True):
            with mock.patch('db_connector_tool.cli.DBConnectorCLI._read_and_split_sql_file', return_value=["SELECT * FROM users"]):
                with mock.patch('db_connector_tool.cli.DBConnectorCLI._execute_sql_statements', return_value=([{"id": 1, "name": "test"}], 1, 0)):
                    with mock.patch('db_connector_tool.cli.DBConnectorCLI._print_execution_summary'):
                        with mock.patch('db_connector_tool.cli.DBConnectorCLI._display_results'):
